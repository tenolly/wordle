    ...
    if (response.ok) {
        ...
        if (result["succses"]) {
            ...
            for (i = 0; i < result["word"].length; ++i) {
                ...
                setTimeout(((i, currentLine) => {
                    let cell = document.getElementById(`line_${currentLine}_cell_${i}`);
                    cell.parentElement.classList.add(`cell--state-${result["result"][i].toLowerCase()}--animation`);
                    let cell_result = document.getElementById(`line_${currentLine}_cell_${i}--result`);
                    cell_result.parentElement.classList.add(`cell--state-${result["result"][i].toLowerCase()}--color`);
                }).bind(null, i, currentLine), 150 * i);
                setTimeout(((i) => {
                    let key = document.getElementById(`button_${result["word"][i].toLowerCase()}`);
                    if (key.className != "cell--state-g--color") {
                        key.className = `cell--state-${result["result"][i].toLowerCase()}--color`;
                    }
                }).bind(null, i), 400 + 150 * i);
            };
            ++currentLine;
            currentCell = 0;
            ...
            if (is_win_game || currentLine == 6) {
                ...
                document.getElementById("result-modal-window__title").innerText = is_win_game ? win_label : lose_label;
                is_playing = false;
            }
        } else {
            let line = document.getElementById(`line_${currentLine}`)
...
addEventListener("keydown", (event) => {
    ...
    else if (languages[language]["alphabet"].test(event.key)) {pressKeybordButton(event.key.toUpperCase())}
});